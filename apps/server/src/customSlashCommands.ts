...
export class CustomSlashCommands extends Context.Service<
  CustomSlashCommands,
...
const makeCustomSlashCommands = Effect.gen(function* () {
  ...
  const emitChange = (state: CustomSlashCommandsState) =>
  ...
  const loadConfigStateFromDisk = Effect.gen(function* () {
    ...
    for (const entry of entries) {
      ...
      const fileInfo = yield* fs.stat(absolutePath).pipe(Effect.orElseSucceed(() => null));
