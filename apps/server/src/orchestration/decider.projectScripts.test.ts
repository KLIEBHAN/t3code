...
describe("decider project scripts", () => {
  ...
  it("emits review prompt events for thread.review.start", async () => {
    const now = new Date().toISOString();
    const initial = createEmptyReadModel(now);
    const withProject = await Effect.runPromise(
      projectEvent(initial, {
        ...
        type: "project.created",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-project-create-review"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-project-create-review"),
        metadata: {},
        payload: {
          projectId: asProjectId("project-1"),
          title: "Project",
    ...
    const readModel = await Effect.runPromise(
      projectEvent(withProject, {
        ...
        type: "thread.created",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-thread-create-review"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-thread-create-review"),
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-1"),
          projectId: asProjectId("project-1"),
    ...
    const result = await Effect.runPromise(
      decideOrchestrationCommand({
        command: {
          ...
          commandId: CommandId.makeUnsafe("cmd-review-start"),
          threadId: ThreadId.makeUnsafe("thread-1"),
  ...
  it("emits compact prompt events for thread.compact.start", async () => {
    const now = new Date().toISOString();
    const initial = createEmptyReadModel(now);
    const withProject = await Effect.runPromise(
      projectEvent(initial, {
        ...
        type: "project.created",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-project-create-compact"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-project-create-compact"),
        metadata: {},
        payload: {
          projectId: asProjectId("project-1"),
          title: "Project",
    ...
    const readModel = await Effect.runPromise(
      projectEvent(withProject, {
        ...
        type: "thread.created",
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-thread-create-compact"),
        causationEventId: null,
        correlationId: CommandId.makeUnsafe("cmd-thread-create-compact"),
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-1"),
          projectId: asProjectId("project-1"),
    ...
    const result = await Effect.runPromise(
      decideOrchestrationCommand({
        command: {
          ...
          commandId: CommandId.makeUnsafe("cmd-compact-start"),
          threadId: ThreadId.makeUnsafe("thread-1"),
    ...
    expect(events[0]?.type).toBe("thread.message-sent");
    if (events[0]?.type === "thread.message-sent") {
      expect(events[0].payload.text).toContain("Create a compact handoff summary for this thread");
