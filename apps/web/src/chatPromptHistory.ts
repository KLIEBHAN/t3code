...
export function shouldResetPromptHistoryBrowsing(input: {
  previousPrompt: string;
  nextPrompt: string;
  previousTerminalContextIds: ReadonlyArray<string>;
  nextTerminalContextIds: ReadonlyArray<string>;
}): boolean {
  ...
  return (
    input.previousTerminalContextIds.length !== input.nextTerminalContextIds.length ||
    input.previousTerminalContextIds.some((id, index) => id !== input.nextTerminalContextIds[index])
  );
}
