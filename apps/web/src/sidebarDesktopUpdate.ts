...
export function useSidebarDesktopUpdate(options: { isElectron: boolean }) {
  const [desktopUpdateState, setDesktopUpdateState] = useState<DesktopUpdateState | null>(null);

  useEffect(() => {
    if (!options.isElectron) return;
  ...
  const showArm64IntelBuildWarning =
    options.isElectron && shouldShowArm64IntelBuildWarning(desktopUpdateState);
