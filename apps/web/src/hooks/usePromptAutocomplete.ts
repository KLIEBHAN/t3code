...
export function usePromptAutocomplete(input: {
  ...
  const generateAutocomplete = useAtomCommand(
    composerAssistEnvironment.generatePromptAutocomplete,
    { reportFailure: false },
  );
  ...
  const runLatestRequest = useCallback(() => {
    ...
    void generateAutocomplete({ environmentId: latest.environmentId, input: latest.request })
      .then((result) => {
  ...
  }, [clearSuggestions, generateAutocomplete, setSuggestions]);
